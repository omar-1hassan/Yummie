...
struct NetworkService {
    ...
    func fetchAllCategories(completion: @escaping(Result<AllDishes, Error>) -> Void) {
        request(route: .fetchAllCategories,
                method: .get,
    ...
    func placeOrder(dishId: String,
        ...
        request(route: .placeOrder(dishId),
                method: .post,
                paramerters: params ,
                completion: completion)
    }
    
    func fetchCategoryDishes(categoryId: String,
                             completion: @escaping(Result<[Dish], Error>) -> Void){
        request(route: .fetchCategoryDishes(categoryId),
                method: .get,
                completion: completion)
    }
    ...
    private func request<T: Decodable>(route: Route,
                                     method: Method,
                                     paramerters: [String: Any]? = nil,
                                     completion: @escaping (Result<T, Error>) -> Void) {
        
        guard let requrst = createRequest(route: route,
                                          method: method,
                                          parameters: paramerters) else {
            completion(.failure(AppError.unknown))
            return
        }
        URLSession.shared.dataTask(with: requrst) { data, response, error in
            ...
            DispatchQueue.main.async {
                //decode our results and send back to user
